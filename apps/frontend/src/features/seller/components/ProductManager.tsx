...
import { createProduct, deleteProduct, fetchSellerProducts, fetchShops } from "../api";
...
export const ProductManager = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormState({ shopId: "", categoryId: "", name: "", description: "", price: 0, stock: 0 });
    },
  });
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-800">Produk</h3>
      ...
      <ul className="grid gap-4">
        {(productsQuery.data ?? []).map((product) => (
          ...
          >
            <div>
              <strong className="text-sm font-semibold text-slate-800">{product.name}</strong>
              <p className="text-sm text-slate-600">Stok: {product.stock}</p>
            </div>
            <button
              type="button"
